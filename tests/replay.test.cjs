...
vm.runInContext(`const clamp=(v,a,b)=>Math.max(a,Math.min(b,v));const S={replayFrames:[{t:-3,x:[0,0,0,0],time:[0,0,0,0]},{t:0,x:[1,2,3,4],time:[0,0,0,0]},{t:10,x:[50,60,70,80],time:[0,0,0,9.5]}],horses:[{},{},{},{time:9.5}]};${declaration('replayDuration')}${declaration('replaySample')}${declaration('replayButtonLabel')}`,context);
...
test('replay control shows replay at the end and continues from pauses',()=>{
  assert.equal(run("replayButtonLabel(true,13,13)"),'❚❚ 暂停');
  assert.equal(run("replayButtonLabel(false,7,13)"),'▶ 继续');
  assert.equal(run("replayButtonLabel(false,13,13)"),'↻ 重播');
});
